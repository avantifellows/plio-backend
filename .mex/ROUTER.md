...
- API/integration tests that follow real user journeys — unit tests cover isolated CRUD only. **Planning is complete: wayfinder map #362 ("green, comprehensive test wall for the Django upgrade") closed 2026-07-13** — all 11 children resolved (decisions #363–#371, provisioning #372–#373; full detail lives in each ticket's resolution comment and the map's Decisions-so-far index). The destination: **seven Ralph-ready workstream issues, run each via `ralph.sh --issue N`** — #374 backend integration suite (pytest harness + API journeys + backend coverage wiring; its harness slice unblocks the fills), #375–#378 per-app unit fills (plio / entries / users / etl; natively blocked-by #374), #379 frontend jest revival + frontend coverage wiring (repo: plio-frontend), #380 Playwright e2e suite + journey manifest (repo: plio-frontend + thin backend caller). #374/#379/#380 are mutually independent. Once the four lanes are green, the per-PR validation runbook (#370, maintainer process — not a Ralph run) drives the chain merges with production checkpoints at 3.2 → 4.2 → 5.2 LTS
