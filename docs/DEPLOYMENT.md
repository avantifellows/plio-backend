...
11. Create and connect a `redis` cluster on AWS Elasticache to your backend app
      ...
      2. Select **Redis** as your Cluster engine. In the checkbox below, choosing between *Cluster mode enabled* or *Cluster mode disabled* is your choice. Plio uses *Cluster mode disabled*. More details on cluster mode [here](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/Replication.Redis-RedisCluster.html)
      ...
      4. Fill all the details in the **Redis Details** section. Some pointers below:
         ...
         7. Select **Advanced Redis settings** and fill in the details as mentioned below:
            1. For *Subnet group* you can create a new group or use an existing one. For staging, subnets were created in step `2.vii`. You can create a new subnet group, name it accordingly and add the subnets (created in step `2.vii`) to that group. A VPC was also created in step `2`. Add that VPC ID in the appropriate selectbox. *NOTE*: If you selected Multi-AZ checkbox in step `11.iv.f`, then the subnet group will require two or more subnets in the group. For example, in staging environment for Plio, three subnets have been created and added to the group.
         ...
         9. Grant access to your redis cluster by following the steps below-
            ...
            4. Choose the Inbound tab, and then do the following:
               ...
               5. Choose **Save**.
         10. Connect the endpoint of your redis instance to the `plio-backend-staging` service that was created inside the `plio-staging-cluster` in Step `10`.
