...
- **Soft-delete-everywhere has exceptions** — the rule holds for product data, but the etl app's `BigqueryJobs` (the superuser-gated `bigquery-jobs` BigQuery-sync bookkeeping table) sets `_safedelete_policy = HARD_DELETE`, so destroy genuinely removes the row even though its viewset docstring says "Soft delete". Pinned as-is, not a bug to fix. The same surface has an anonymous-caller crash quirk: it stacks no `IsAuthenticated` before its superuser check, so with `UNAUTHENTICATED_USER: None` a guest request raises `AttributeError` (HTTP 500), not a 401/403.
