...
def get_plios_list(request: Request):
    response = {"all_plios": get_all_plios()}
...
def get_plios_df(request: Request):
    ...
    keep_test_plios = json.loads(request.GET.get("keep_test_plios", "False").lower())

    if preprocess:
        # load video durations
        plios_df["video_duration"] = get_video_durations_from_ids(plios_df["video_id"])
...
def get_plio(request: Request):
    ...
    data = requests.get(
        DB_QUERIES_URL + URL_PREFIX_GET_PLIO, params={"plio_id": plio_id}
    )
    ...
    if session_id != 0:
        session_data = requests.get(
            DB_QUERIES_URL + URL_PREFIX_GET_SESSION_DATA,
            params={
                "plio_id": plio_id,
                "user_id": user_id,
...
def get_plio_config(plio_id: str):
    ...
    if plio_config.status_code == 404:
        return HttpResponseNotFound(
            f"<h1>plio-config for plio {plio_id} not found</h1>"
        )
